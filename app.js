...
function buildSkillExpTab() {
  ...
  if ($("exp-current")) {
    $("exp-current").innerHTML = buildOpts(290);
    $("exp-target").innerHTML = buildOpts(290);
    $("exp-current").value = "1";
    $("exp-target").value = "290";
...
function updateSkillExp() {
  ...
  const expCurVal = EXP_CUMULATIVE[expCur - 1] || 0;
  const expTgtVal = EXP_CUMULATIVE[expTgt - 1] || 0;
  const expNeeded = Math.max(0, expTgtVal - expCurVal);
  const expColor = expNeeded === 0 && expTgt <= expCur ? "var(--text-dim)" : "var(--blue)";
  ...
  const skCurVal = SKILL_CUMULATIVE[skCur - 1] || 0;
  const skTgtVal = SKILL_CUMULATIVE[skTgt - 1] || 0;
  const skNeeded = Math.max(0, skTgtVal - skCurVal);
  const skColor = skNeeded === 0 && skTgt <= skCur ? "var(--text-dim)" : "var(--green)";
